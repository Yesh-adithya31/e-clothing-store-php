...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $userID = $_POST['userid'];
    $stmt = $conn->prepare("SELECT COUNT(*) AS cart_count, SUM(product_master.price * shopping_cart.qty) AS total_Sum FROM user_has_shopping_cart LEFT JOIN shopping_cart ON user_has_shopping_cart.shopping_cart_id = shopping_cart.shopping_cart_id LEFT JOIN product_master ON shopping_cart.product_master_id = product_master.product_master_id LEFT JOIN product_details ON product_master.product_detail_id = product_details.product_details_id WHERE user_has_shopping_cart.user_id = :userid");
