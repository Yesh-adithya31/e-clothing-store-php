...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT * FROM variation_option LEFT JOIN variation ON variation_option.variation_id = variation.variant_id LEFT JOIN category ON variation.category_id = category.id WHERE category.is_Deleted = 0");
    $stmt->execute();
